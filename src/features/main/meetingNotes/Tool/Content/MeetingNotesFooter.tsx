...
import { MeetingNotesStep } from "../../../../../types/meetingNotesTypes";
...
type MeetingNotesFooterProps = {
  step: MeetingNotesStep;
  setCurrentStep: (step: MeetingNotesStep) => void;
  nextStep: () => void;
...
const MeetingNotesFooter: React.FC<MeetingNotesFooterProps> = ({
  step,
  nextStep,
  isNextButtonVisible,
  setCurrentStep,
  ...
  const [meetingNotesFromValues, setMeetingNotesFormValues] =
    useAtom(meetingNotesFormAtom);
  const [snackbarOpen, setSnackbarOpen] = useState(false);
  const isDisabled =
    meetingNotesFromValues.objectives === "" &&
    !meetingNotesFromValues.file?.fileName;
  ...
  const filteredSteps = steps.filter(
    (step) =>
      !(
        meetingNotesFromValues.type.name === "General Meeting Notes" &&
        step.label === "Structure"
      )
  );

  const handleStepClick = (index: number) => {
    if (index === 0) {
      setMeetingNotesFormValues(meetingNotesFromValues);
    }
  ...
  const handleReturnToFirstStep = () => {
    ...
    setMeetingNotesFormValues(initialMeetingNotesValues);
    setDroppedItems([]);
  };
    ...
    <>
      ...
      >
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            <ProgressList>
              {filteredSteps.map((currStep, index) => {
                const status =
...
export default MeetingNotesFooter;
